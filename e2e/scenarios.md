...
| `auth-sign-in-form` | must | From the welcome screen, the sign-in screen opens with its fields, pressing Sign in with the form blank names each missing field beside it under a count of the problems, the Collection value toggles to an editable input, and an unreachable server surfaces an inline error. |
