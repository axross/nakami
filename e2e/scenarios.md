...
[`collections.record-field-editor`](#collectionsrecord-field-editor); the first of
those two needs nothing the other eight do not, and the second needs only that the
runner can answer a native alert, so both belong with them rather than with the three
below.
