...
afterEach(() => {
	client.clear();
	// the offline test below flips this process-wide; put it back so it cannot
	// reach another test.
...
describe("getUpdateRecordFieldMutationOptions", () => {
	...
	// the pending-write queue awaits this mutation and holds its mutex while it
	// does. a mutation paused for want of connectivity never settles, so a
	// paused one here would hold that mutex for the rest of the session.
	it("never pauses for want of connectivity, so the send always settles", async () => {
		onlineManager.setOnline(false);

		const mutation = client.getMutationCache().build(client, optionsFor());

		await expect(
			mutation.execute({ fieldName: "title", value: "Renamed" }),
	...
	// `scope` is the other half of that: it is this library's serialiser, and a
	// scoped mutation left pending blocks every later one sharing its id. the
	// queue's mutex already serialises, so a second serialiser under it could
	// only deadlock the first.
	it("leaves serialization to the queue rather than taking a scope", () => {
		expect(optionsFor().scope).toBeUndefined();
	});
