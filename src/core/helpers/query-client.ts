...
onlineManager.setEventListener((setOnline) => {
	...
	Network.getNetworkStateAsync()
		...
		.catch((error: unknown) => {
			...
			// fallback, which the next change event corrects. It is still logged,
			// so the breadcrumb is there if a later error traces back to it.
			logger.warn("Could not read the launch-time network state.", { error });
		});
