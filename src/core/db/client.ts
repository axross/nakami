// The shared on-device database client. Nothing in src/ imports it yet:
// everything the app persists today goes to the keychain
// (src/auth/helpers/session-storage.ts and src/auth/helpers/last-server-url.ts)
// and everything it reads comes from TanStack Query over the Payload REST API.
...
// never run.
//
// Because there is no importer, ./client.test.ts is the only thing that
