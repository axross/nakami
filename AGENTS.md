...
- MUST keep the server URL out of a query key. Server and user are one authentication-session pair, so the user id identifies the tenant on its own; a `queryFn` reads the URL from the session alongside the token, exactly as it reads the token. This knowingly diverges from the installed skill's tenancy-dimension MUST and rests on the sign-out eviction above — see [Known Deviations](#known-deviations-from-the-installed-skills) before relaxing either.
