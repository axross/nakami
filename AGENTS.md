...
- MUST style with `StyleSheet.create((theme) => ({ … }))` from `react-native-unistyles` and consume tokens from `src/common/constants/style.ts`:
  ...
  - `theme.gap.*` (`xs` 8 / `sm` 12 / `md` 16 / `lg` 24 / `xl` 32) for scale spacing only — never for a radius or a border width, each of which has its own family below.
  - `theme.radius.*` (`sm` 8 / `md` 12 / `lg` 16 / `pill` 999) for every corner, picked by role: `sm` for inset marks and placeholder bars, `md` for the default surface corner (cards, inputs, buttons, menu-group ends), `lg` for large marks (the message-state icon plate, the account avatar), `pill` for a fully rounded chip.
  - `theme.borderWidth.*` (`hairline` = `StyleSheet.hairlineWidth` / `thin` 1) for every border, divider height, and flush-row `rowGap`. Every separator and outline in the app is a `hairline`.
  - `theme.duration.*` (`fast` 150 / `base` 250 / `slow` 700) and `theme.easing.*` (`standard`) for every animation, read via `useUnistyles()` where the value is passed to reanimated rather than to a stylesheet.
  - `theme.fonts.*` (`heading` / `paragraph` / `label` / `monospace`).
- MUST NOT hard-code a color, a spacing, radius, border-width, duration, or easing value. Inlined numeric literals are limited to a text style's `fontSize` (paired with a `theme.fonts.*` family, since the theme carries no size scale — tracked as issue #71), fixed element dimensions, and ratios such as an opacity or a `flex` value.
