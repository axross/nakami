...
- **Three jobs:** `prebuild` runs `expo prebuild` and caches the generated `android/` project keyed by a checksum of `app.json` + `package-lock.json` (skipped on a cache hit), handing it to `build_android` via an artifact; `build_android` signs the release APK and uploads it to Firebase; and `report` writes the install link to the run summary and, when the `pr` input is given, comments it on that PR. Requires the `ANDROID_*`, `FIREBASE_*`, and `SENTRY_AUTH_TOKEN` secrets above.
