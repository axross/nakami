...
Unit tests (Jest via jest-expo, colocated with their subject) cover helpers, schemas, and component behavior; Maestro flows in `e2e/` assert whole user journeys on a simulator, tracked against the journey catalog in `e2e/scenarios.md` — one row per journey carrying a dotted id, a title, an area, and a priority, which the coverage gate joins to the `scenario:` and facet tags each flow declares. [`docs/conventions/end-to-end-testing.md`](./docs/conventions/end-to-end-testing.md) states that tag vocabulary, what a green gate does and does not prove, and the Payload fixture an authenticated flow needs. Merges are gated by CI ([`merge-checks.yml`](./.github/workflows/merge-checks.yml)), which runs on every PR update as five independent, parallel jobs — **Lint** (Biome), **Typecheck** (TypeScript), **Unit Tests** (Jest), **E2E Scenario Coverage**, and **Docs** (the `docs/` validators below) — so a red check names exactly one failing tool.
...
The data layer is scaffolding until then: `src/core/db/schema.ts` declares no table, and nothing imports `src/core/db/client.ts` but its own colocated smoke test. That test **mocks `expo-sqlite`, and therefore leaves the native database open unverified**. What it does cover is narrow — `drizzle()` still returns an object carrying the `select`, `insert`, `update`, and `delete` builders, so a Drizzle bump that drops or renames one turns the suite red, while one that keeps those names and changes what they build does not. Everything the app persists today goes to the keychain, and everything it reads comes from the Payload REST API through TanStack Query; [`docs/conventions/data-layer.md`](./docs/conventions/data-layer.md) holds the rules a read has to satisfy once that changes.
...
**`npm audit` reports 24 findings, and that is the expected steady state.** 8 moderate and 16 high, every one of them in build-time tooling rather than in anything that ships to a device. A count that differs from 24 means something new arrived — investigate it rather than assuming it is one of these. The `overrides` block in [`package.json`](./package.json) clears 9 of the 11 advisory roots that preceded it; two remain, for different reasons.

| Remainder    | Findings                                                                                                                              | Why it stays                                                                                                                                                                     |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `image-size` | 16 high — 1 root plus 15 cascade, through `metro` → `@expo/metro` → `expo` and `@react-native/metro-config` → `react-native`          | **No published fix exists at any version.** The latest release is 2.0.2 and the advisories cover `<=2.0.2`, so there is nothing to override to. Accepted, not pending.            |
| `uuid`       | 8 moderate — 1 root plus 7 cascade, through `xcode` → `@expo/config-plugins` → `expo-splash-screen`                                    | A fixed release exists (11.1.1), but only by forcing past the `^7.0.3` that `xcode` declares — and npm does not hold that override stably. See the rejected override below.       |

**Both are build-time exposure, not runtime.** `image-size` is reached through Metro's asset pipeline and `uuid` through `expo prebuild`'s iOS project generation. Both run on a developer machine or a CI runner; neither is reachable from the app entry point or bundled into a device build.

`npm audit --omit=dev` does **not** clear them — measured, not assumed: it reports the same 24 findings (8 moderate, 16 high). Both roots hang off production dependencies (`expo` and `expo-splash-screen`), so the flag does not separate them out even though the code only ever runs at build time.

**Re-check when** `image-size` publishes a release above 2.0.2, or an Expo SDK bump moves Metro off it; and for `uuid`, when `xcode` widens its declared range. Drop the corresponding row here once either clears.

**A forced `uuid` override was attempted and rejected.** `"uuid@^7.0.3": "^11.1.1"` resolves, but npm 10.9.8 does not resolve it _reproducibly_: repeated `npm install` alternates between hoisting `uuid@11.1.1` (override applied) and nesting `uuid@7.0.3` (override lost), so the lockfile never settles and `npm ci` can reinstate the vulnerable version. It was dropped rather than shipped in that state. The sibling forced override that did survive — `"esbuild@~0.18.20": "^0.25.12"`, the line `drizzle-kit` loads `drizzle.config.ts` through — is stable across repeated installs and is checked by `npm run db-migrate:generate`.

**Editing the `overrides` block needs a lockfile nudge.** `npm install` applies a changed override to the dependency _edge_ but leaves an already-locked nested package where it is, so `npm ls` reports it `invalid: … overridden` and the audit count does not move. Delete the stale entries from `package-lock.json` along with the matching `node_modules` subtree, reinstall, then confirm with `npm ls` that the edge reads `overridden` and no `invalid` remains. Every key there is version-scoped (`name@range`) so it binds only the dependency line it is meant to — a bare `brace-expansion` key would force 5.x into `minimatch@3`, whose CommonJS entry expects a callable export, and break every suite.
...
**`metro.config.js` must keep building its config from `@sentry/react-native/metro`'s `getSentryExpoConfig`.** That factory is what prepends the runtime Debug ID module to every bundle Metro produces, and reverting it to Expo's `getDefaultConfig` silently disables Sentry Debug IDs: the build still succeeds, the config plugin still runs, events still arrive, and every production stack trace resolves to minified frames. A `debugId` present in an emitted source map does not prove the wiring is intact — Expo's own serializer stamps that either way — so the evidence to look for is a `sentry-dbid-<uuid>` marker in the bundle itself, matching that map's `debugId`. The `react-native-svg-transformer` customization has to stay layered after the factory call.
