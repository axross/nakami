...
- **Installed copies** — the 23 general capabilities from [`axross/skills`](https://github.com/axross/skills), pinned by `skills-lock.json`. Nothing there is hand-written, and a hand-edit is discarded by the next reinstall. A rule that is wrong or missing in one of them belongs upstream: open a feature request against `axross/skills` rather than patching the installed copy.
