...
**`npm audit`'s high findings are known, and the expected count is 27.** All 27 are one advisory — [GHSA-mh99-v99m-4gvg](https://github.com/advisories/GHSA-mh99-v99m-4gvg), an unbounded-expansion out-of-memory crash in `brace-expansion` — reached through the Jest 29 tree that `jest-expo` pins. One entry is the package itself; the other 26 are npm listing every package that depends on a vulnerable version, up the chain `minimatch` → `glob` / `test-exclude` → `babel-plugin-istanbul` → `babel-jest` → the `@jest/*` packages → `jest` → `@testing-library/react-native` → `expo-router`, plus `@react-native/jest-preset` → `react-native`.

It is accepted rather than unexamined, on three findings:

- **It does not reach the shipped app.** The affected code is Node-side glob matching that runs only under the test runner; an `expo export` contains no executable code from `brace-expansion`, `minimatch`, `glob`, `test-exclude`, or `@react-native/jest-preset`. Note that `npm audit --omit=dev` does **not** clear these (it still reports 25 high) — `expo-router` and `react-native` declare `@testing-library/react-native` and `@react-native/jest-preset` in their own `dependencies`, so the tree counts as production to npm even though Metro never bundles it. Bundle reachability, not the `devDependencies` split, is what makes this dev-only in practice.
- **There is no compatible fix to take.** The advisory covers every `brace-expansion` below 5.0.8 — the 1.x line included — and 5.x is an ESM/CJS rewrite whose CommonJS entry exports a namespace object (`{ expand, … }`) instead of the callable function `minimatch@3.1.5` obtains from `require("brace-expansion")` and invokes. A blanket override therefore breaks the suite instead of fixing it.
- **The documented pins forbid moving the tree.** `npm audit fix --force` "resolves" it by downgrading `expo-router` to 5.1.11, `react-native` to 0.84.1, `jest-expo` to 32.x, and `jest` to 25.x. [`jest.config.cjs`](./jest.config.cjs) holds `@testing-library/react-native` at v13 (expo-router 57's `renderRouter` calls RNTL's `render` synchronously, and v14 made it async), and `jest-expo` tracks the Expo SDK 57 line.

The `overrides` block in `package.json` covers what a compatible fix exists for: `fast-uri` is resolved outright ([GHSA-v2hh-gcrm-f6hx](https://github.com/advisories/GHSA-v2hh-gcrm-f6hx), host confusion), and `brace-expansion`'s 5.x line is held at 5.0.8, removing the one copy of it that could be patched in place. Together those took the count from 28 high to 27. Both keys are version-scoped (`name@range`) so they bump only the lines they are meant to and leave the incompatible 1.x copies alone.

**Re-check this at each Expo SDK bump**, when the pins move anyway — the finding clears once the Jest tree reaches a `minimatch` consuming `brace-expansion` ≥ 5.0.8, or once upstream backports the fix to 1.x. CI deliberately does not gate on `npm audit`: an `--audit-level=high` job would fail on every run until then, so it would need an allowlist to tell a reader anything this note does not. The 18 `moderate` findings are outside that record and trace to three advisories in build-time toolchains — `esbuild` under `drizzle-kit`, `@hono/node-server` under `expo-mcp`, and `uuid` under `expo-splash-screen`'s prebuild config plugin.
