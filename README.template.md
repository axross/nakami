...
| ---- | ---- |
| Language | TypeScript |
| App framework / runtime | Expo (React Native) |
| Package manager | npm |
| Linting & formatting | Biome / Biome |
| Unit tests | Jest (jest-expo) with React Native Testing Library <!-- INIT:OPTIONAL key=UNIT_TESTS — fill the token OR delete this row if the project has no unit suite. --> |
...
| Hosting | EAS (Expo Application Services) <!-- INIT:OPTIONAL key=HOSTING — fill the token OR delete this row if the project has no hosting platform yet. --> |
