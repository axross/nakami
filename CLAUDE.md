...
- MUST write responsive style values against the `breakpoints` exported from the same file (`xs` 0 / `sm` 380 / `md` 768) and registered in `src/unistyles.ts`, which also sets `adaptiveThemes: true` — the theme follows the OS color scheme with no in-app toggle, so a scheme switch swaps the theme object underneath a mounted screen.
...
- **Two existing query keys use the trailing-filter tenancy shape** the TanStack Query skill forbids. See [Server State](#server-state); new code follows the skill, and migrating the two is tracked as issue #67.
