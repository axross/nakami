...
| Version identifier | Each preview APK's `versionName` encodes the source it was built from — `<semver>-pr-<n>` for a `pr`-input dispatch, else `<semver>-<short-sha>` (`<semver>` = `app.json`'s `expo.version`). Visible in Firebase App Distribution and Android app-info, so a tester can match an install to its PR/commit. Resolved in `app.config.ts` from the workflow's `PREVIEW_VERSION_NAME` and baked into the generated native project at `expo prebuild` (so the `prebuild` cache key includes it); a `Verify APK versionName` step reads it back from the assembled APK with `aapt2` and fails the build on a mismatch. Only the preview build sets it — `dev`/`run`/local builds keep the plain `app.json` version. |
