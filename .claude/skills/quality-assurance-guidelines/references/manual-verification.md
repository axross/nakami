...
| Deep-link scheme, app config, or store metadata (`app.json`) *(optional)* | The deep link was opened from outside the app (e.g., `npx uri-scheme open`), or the config change was confirmed in the resolved config (`npx expo config`) |
