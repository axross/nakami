...
Apply these rules to verify the change does not introduce or paper over test flakiness. When the project's Maestro config is set so flakiness fails the suite (e.g., repeating each test and failing on flaky results rather than silently retrying), an intermittent test is a failure, not a warning.
...
- MUST flag a Critical when the diff modifies the Maestro config to weaken anti-flake settings (repeat-each, fail-on-flaky, forbid-focused-tests) or to add retries. Defer the change to the human owner per the project's code-review guideline (escalation rules).
