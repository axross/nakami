...
Apply these rules to verify the change keeps the project's error-propagation model and structured-logging discipline intact. Defer the developer-facing rules to the project's observability guidelines — this file is the **reviewer's** flagging checklist. Throughout, `reportError(...)` denotes the project's error-reporting call (it maps to Sentry's capture function if the project has one), and `logger` denotes the project's structured logger (react-native-logs).
...
- MUST flag a Major when a new `try`/`catch` is placed inside a nested helper rather than at the root call site (the screen, query, or mutation entry point). The project rule per the project's observability guidelines (error-handling rules) is "let errors propagate to the root call site".
...
- MUST flag a Major when `@sentry/react-native` is imported anywhere outside `~/core/helpers/error-reporting.ts` — app code goes through the `reportError`/`wrapRootComponent` wrapper.
...
- MUST flag a Major when an info-level log is emitted for a high-frequency operation (e.g., per component render, per-iteration inside a tight loop). Log at the boundary of the operation, not inside it.
...
> **Dormant until session replay / tracing is enabled in `Sentry.init`** —
> remove this banner when either lands, making the rules unconditional.
...
- SHOULD flag a Minor recommendation to lower the trace sampling rate when a new high-traffic surface is added — full sampling will hit the error tracker's quotas.
...
- SHOULD flag a Major when a new external `fetch`/network call inside a query or mutation lacks a timeout (e.g., an abort signal with a deadline). A hung dependency stalls the screen it feeds.
