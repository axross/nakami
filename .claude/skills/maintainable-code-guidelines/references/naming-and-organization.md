...
| Data-layer realm | Lives under `src/core/db/` only (schema, client, generated migrations) |
...
- MUST flag a new file placed under `src/core/db/` that does not belong there (anything other than schema, client, and generated migrations), per the project's development guidelines (change-management rules).
...
- MUST flag identifier-naming inconsistency within the changed file's neighborhood. Examples to flag:
  ...
  - A logging-module label/tag that duplicates an existing module's label when the project requires unique per-module labels, per the project's observability guidelines (logging rules).
