...
- MUST present every design round as a published Artifact — at both wireframe and high fidelity, for options rounds and confirmation rounds alike — as the round's design source of truth.
- MUST build each round's Artifact from the fidelity's kit as the required starting point — a **wireframe round** from the wireframe kit ([wireframe-kit.md](./wireframe-kit.md) + [wireframe-kit.html](../assets/wireframe-kit.html)); a **high-fidelity round** from the hi-fi design kit ([hifi-design-kit.md](./hifi-design-kit.md) + [hifi-design-kit.html](../assets/hifi-design-kit.html)) — and consult the harness's artifact-design guidance (in Claude Code, the `artifact-design` skill) for general page craft before publishing. The kits own *how* each fidelity is built; this section owns *which* fidelity a round requires.
- MUST honor the fidelity contract: a wireframe round's Artifact stays at breadboard fidelity (greyscale — regions, hierarchy, flow), and a high-fidelity round's Artifact renders the app's design tokens from `src/common/constants/style.ts` in both light and dark. Each kit owns what its fidelity looks like, including the specifics; this section owns only which fidelity a round requires.
- MUST shape the Artifact to the round: an options round renders its at-least-three candidates so they compare side by side, each labeled and carrying its rationale and trade-offs with exactly one marked `(Recommended)`; a confirmation round renders the single already-approved direction. Cover both light and dark themes and the device sizes where the design differs.
...
1. Build the mockup as a self-contained page in a scratch location outside the repository checkout (the harness scratchpad), starting from the hi-fi design kit ([hifi-design-kit.md](./hifi-design-kit.md)) and following the harness's artifact-design guidance, covering both themes and the device sizes where the design differs.
