...
- MUST build each round's Artifact from the fidelity's kit as the required starting point — a **wireframe round** from the wireframe kit ([wireframe-kit.md](./wireframe-kit.md) + [wireframe-kit.html](../assets/wireframe-kit.html)); a **high-fidelity round** from the hi-fi kit ([hifi-kit.md](./hifi-kit.md) + [hifi-kit.html](../assets/hifi-kit.html)) — and consult the harness's artifact-design guidance (in Claude Code, the `artifact-design` skill) for general page craft before publishing. The kits own *how* each fidelity is built; this section owns *which* fidelity a round requires.
...
1. Build the mockup as a self-contained page in a scratch location outside the repository checkout (the harness scratchpad), starting from the hi-fi kit ([hifi-kit.md](./hifi-kit.md)) and following the harness's artifact-design guidance, covering both themes and the device sizes where the design differs.
