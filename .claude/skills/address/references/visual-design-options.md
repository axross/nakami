...
- MUST build each round's Artifact from the project's wireframe kit ([wireframe-kit.md](./wireframe-kit.md) + [wireframe-kit.html](./wireframe-kit.html)) as the required starting point, and consult the harness's artifact-design guidance (in Claude Code, the `artifact-design` skill) for general page craft before publishing. The kit owns *how* each mode is built; this section owns *which* mode a round requires.
- MUST honor the kit's fidelity contract: build a wireframe round's Artifact in the kit's breadboard (greyscale) mode and a high-fidelity round's Artifact in the kit's real-token mode, which renders the app's design tokens from `src/common/constants/style.ts` in both light and dark. The kit owns what each fidelity looks like, including the breadboard-fidelity specifics; this section owns only which mode a round requires.
...
1. Build the mockup as a self-contained page in a scratch location outside the repository checkout (the harness scratchpad), starting from the wireframe kit's high-fidelity mode ([wireframe-kit.md](./wireframe-kit.md)) and following the harness's artifact-design guidance, covering both themes and the device sizes where the design differs.
