...
- MUST build each round's Artifact from the fidelity's kit skill as the required starting point — a **wireframe round** per the project's wireframe design guidelines; a **high-fidelity round** per its high-fidelity UI design guidelines — and consult the harness's artifact-design guidance (in Claude Code, the `artifact-design` skill) for general page craft before publishing. Those skills own *how* each fidelity is built; this section owns *which* fidelity a round requires.
...
1. Build the mockup as a self-contained page in a scratch location outside the repository checkout (the harness scratchpad), starting from the project's high-fidelity UI design guidelines and following the harness's artifact-design guidance, covering both themes and the device sizes where the design differs.
