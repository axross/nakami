...
Apply this reference when drafting or reviewing the **System design** subsection (nested under Functional requirements) and the top-level **Non-functional requirements** section of a spec — data flow, module boundaries, alternatives considered, and measurable constraints, described at the specification level, not the implementation itself. Sourced from design-doc and architecture-decision-record practice: [Design Docs at Google](https://www.industrialempathy.com/posts/design-docs-at-google/), the [C4 model](https://c4model.com/), [arc42](https://arc42.org/overview), [Michael Nygard's ADR format](https://github.com/joelparkerhenderson/architecture-decision-record/blob/main/locales/en/templates/decision-record-template-by-michael-nygard/index.md), the [Rust RFC template](https://github.com/rust-lang/rfcs/blob/master/0000-template.md), [AWS's ADR guidance](https://docs.aws.amazon.com/prescriptive-guidance/latest/architectural-decision-records/adr-process.html), and [GitLab's documentation ladder](https://docs.gitlab.com/charts/architecture/decision-making/). Implementation mechanics for these same concerns — actual module placement, file layout, routing — stay owned by the project-structure skill; this reference owns only how to *describe* system-design decisions in a spec.

## When to Include a System Design Section

Not every change earns a System design section, but two independent things can each earn it. arc42 treats every one of its architecture sections as optional by design, included only when the project's scale warrants it, and GitLab's documentation ladder makes the same point operationally: a single-team, low-blast-radius change is often fully documented by a good commit message, and work that crosses team or module boundaries escalates to a design doc. Shape Up generalizes this into "appetite" — the amount of process, and by extension documentation, a piece of work deserves is set by its size and reversibility up front. But blast radius is not the only reason a spec needs a System design section: a change confined to one module can still turn on **intricate internal mechanics** — a subtle ordering, an invariant, a non-obvious state transition — that a future engineer would need the *why* of, even though nothing crosses a boundary.

These are two **disjunctive** triggers: each suffices on its own, and neither is required for the other. The section is included when the change crosses an architecture boundary **or** when its mechanics are intricate enough to need explaining, and it is omitted only when **neither** holds — a self-contained change with straightforward mechanics.

**Guidelines:**

- MUST include a System design section when **either** trigger applies, each sufficient alone: (a) the **architecture-boundary trigger** — the change crosses a module, service, or team boundary, or is expensive to reverse; or (b) the **intricate-mechanics trigger** — the change's internal mechanics are subtle enough (a non-obvious ordering, invariant, or state transition) that a future engineer would need to know *why* this shape was chosen, even for a minor-scoped, single-module change.
- MUST omit the section only when **neither** trigger applies — a self-contained change with no shared-state impact and straightforward mechanics — stating the one-line reason in place of the section.
- MUST NOT treat the two triggers as conjunctive: a change that meets the intricate-mechanics trigger earns the section even though it crosses no boundary, and a boundary-crossing change earns it even when its mechanics are simple.
- SHOULD size the section to the change's appetite rather than a fixed template — a narrow but intricate change earns a focused paragraph on the mechanic; a new shared-state model or service boundary earns more.
...
- MUST NOT prescribe file layout, routing, or module-placement mechanics that belong to the project-structure skill.
