...
description: Conventions for Jest (jest-expo) with React Native Testing Library unit tests. Covers test framework configuration, explicit test-API imports, colocated test files, describe/case naming and grouping conventions, behavior-focused test design, fixture quality, AHA test abstraction, mocks and fakes, async assertions, snapshot discipline, schema/codec tests, type-only modules, and when unit tests should yield to integration or e2e coverage.
...
Use this skill for Jest (jest-expo) with React Native Testing Library-based unit tests. Unit tests are valuable when they exercise a small exported contract quickly, independently, and from a caller's point of view. They are harmful when they overfit implementation details or replace higher-confidence integration/e2e coverage.
...
- implementation self-review or code review for Jest (jest-expo) with React Native Testing Library unit tests
...
- MUST colocate Jest (jest-expo) with React Native Testing Library tests following the project's colocated unit-test file naming convention unless an existing local pattern requires a different location.
- MUST import the test framework's APIs explicitly if Jest (jest-expo) with React Native Testing Library requires it, rather than relying on implicit global-scope symbols.
