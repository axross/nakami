...
Use this checklist during implementation self-review or code review for Jest (jest-expo) with React Native Testing Library unit tests. It complements the project's quality-assurance guidelines, which owns verification evidence and residual-risk reporting.
...
- MUST verify every new or changed spec imports the test framework's APIs explicitly if Jest (jest-expo) with React Native Testing Library requires it.
